...
# The validated old optimization stack is embedded here.  No external
# opt_stack.env or script outside code/Megatron-LM is required.

CURRENT_TIME=$(date "+%Y%m%d_%H%M%S")
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MEGATRON_PATH="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
WORK_HOME="$(cd "${MEGATRON_PATH}/../.." && pwd)"
cd "${WORK_HOME}"

PATCH_HOME=${PATCH_HOME:-"${WORK_HOME}/code/megatron-lm-musa-patch"}
...
check_file "${MEGATRON_PATH}/pretrain_minicpm5_musa.py"
