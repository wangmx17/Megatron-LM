...
# @torch.compile
def zeropower_via_newtonschulz5(G, steps, coefficient_type):
    """
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, param_groups, lr=2e-2, weight_decay=0.1,
                 ...
                 adamw_betas=(0.95, 0.95), adamw_eps=1e-8, qkv_split_shapes=None, 
                 muon_split_qkv=False, muon_coefficient_type="simple"):
        ...
        self.muon_split_qkv = muon_split_qkv
    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                # calc update
                if self.muon_split_qkv and getattr(p, "is_qkv", False):
                    num_query_groups = ns_input.shape[0] // sum(self.qkv_split_shapes)
                    qkv_grads = torch.split(
                        ns_input.view(num_query_groups, sum(self.qkv_split_shapes), -1),
                        self.qkv_split_shapes,
                        dim=1,
                    )
                    qkv_grads = [g.reshape(-1, ns_input.shape[-1]) for g in qkv_grads]
                    qkv_grads = [
                        zeropower_via_newtonschulz5(g, steps=ns_steps, coefficient_type=self.muon_coefficient_type).view(
                            num_query_groups, -1, ns_input.shape[-1]
                        )
                        for g in qkv_grads
                    ]
                    update = torch.cat(qkv_grads, dim=1).view(ns_input.shape)
                else:
                    update = zeropower_via_newtonschulz5(ns_input, steps=ns_steps, coefficient_type=self.muon_coefficient_type)
