...
class TEDotProductAttention(te.pytorch.DotProductAttention):
    ...
    def forward(
        ...
        # Apply elementwise attention output gate if gate_score is provided
        if gate_score is not None:
            # gate_score shape: [sq, b, np, hn]
            # core_attn_out from TE is already reshaped based on qkv_format:
            # - 'sbhd' (default): [sq, b, h] where h = np * hn
            # - 'bshd' (with rope_fusion): [b, sq, h] where h = np * hn
            # Need to reshape gate_score to match
            # gate_score: [sq, b, np, hn] -> [sq, b, np*hn]
            sq, b, np, hn = gate_score.shape
            gate_score_flat = gate_score.view(sq, b, np * hn)
            
            if qkv_format == 'bshd':
                # core_attn_out is [b, sq, h], transpose gate_score to [b, sq, h]
                gate_score_flat = gate_score_flat.transpose(0, 1)  # [b, sq, h]
                core_attn_out = core_attn_out * torch.sigmoid(gate_score_flat)
            else:
                # core_attn_out is [sq, b, h], gate_score_flat is also [sq, b, h]
                if qkv_format == 'thd':
                    core_attn_out = core_attn_out.unsqueeze(dim=1)
                core_attn_out = core_attn_out * torch.sigmoid(gate_score_flat)
