...
from megatron.core.transformer.nsa_attention import (
    NSADotProductAttention,
    NSAHybridDotProductAttention,
)
...
def get_gpt_nsa_decoder_block_spec(
    config: TransformerConfig,
    block_size: int = 64,
    window_size: int = 512,
    sparse_blocks: int = 8,
    ...
    """GPT NSA block spec that respects moe_layer_freq for mixed dense/MoE layers."""
    ...
    if hybrid_threshold is not None:
        nsa_kwargs['hybrid_threshold'] = hybrid_threshold
        get_nsa_spec = get_gpt_layer_with_nsa_hybrid_spec
    else:
        get_nsa_spec = get_gpt_layer_with_nsa_spec

    dense_layer_spec = get_nsa_spec(
        num_experts=None, moe_grouped_gemm=False, **nsa_kwargs,
    )
    moe_layer_spec = get_nsa_spec(
        num_experts=config.num_moe_experts,
        moe_grouped_gemm=config.moe_grouped_gemm,
        **nsa_kwargs,
    )
    ...
    block_spec = TransformerBlockSubmodules(
        layer_specs=local_layer_specs, layer_norm=TENorm
    )
    return block_spec


def get_gpt_layer_with_nsa_spec(
...
def get_gpt_layer_with_nsa_hybrid_spec(
    num_experts: int = None, 
    moe_grouped_gemm: bool = False,
    qk_layernorm: bool = False,
    block_size: int = 128,
        ...
        submodules=TransformerLayerSubmodules(
            self_attention=ModuleSpec(
                ...
                submodules=SelfAttentionSubmodules(
                    linear_qkv=backend.column_parallel_layer_norm_linear(),
                    core_attention=ModuleSpec(
                        module=NSAHybridDotProductAttention,
                        params={
                            "block_size": block_size,
                            "window_size": window_size,
                            "topk": sparse_blocks,
