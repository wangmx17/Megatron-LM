...
try:
    from megatron.core.models.common.embeddings.fused_rope_thd import fused_rope_thd as _triton_rope_thd
except ImportError:
    _triton_rope_thd = None
...
def apply_rotary_pos_emb(
    ...
    if config.apply_rope_fusion:
        if cu_seqlens is None:
            ...
            if getattr(config, 'use_triton_rope', False) \
                    and _triton_rope_thd is not None \
                    and not config.rotary_interleaved:
                return _triton_rope_thd(
                    t, cu_seqlens, freqs,
                    cp_size=cp_group.size(), cp_rank=cp_group.rank(),
                )
