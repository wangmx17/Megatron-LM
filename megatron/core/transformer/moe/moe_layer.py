...
class MoELayer(BaseMoELayer):
    ...
    def router_and_preprocess(self, hidden_states: torch.Tensor):
        ...
        Returns:
            Tuple of (hidden_states, probs, residual, routing_info) where routing_info
            is (routing_probs, routing_map) if health monitoring is enabled, else None.
        """
        residual = hidden_states
        probs, routing_map = self.router(hidden_states)
        # Capture routing info for health monitoring before dispatch_preprocess consumes it
        routing_info = None
        if self.config.moe_expert_health_monitoring and self.training:
            routing_info = (probs.detach(), routing_map.detach())
        hidden_states, probs = self.token_dispatcher.dispatch_preprocess(
            hidden_states, routing_map, probs
        )
        return hidden_states, probs, residual, routing_info
    ...
    def routed_experts_compute(
        ...
        routing_info: Optional[tuple] = None,
    ):
        ...
        Args:
            hidden_states: Dispatched hidden states.
            probs: Routing probabilities.
            residual: Residual connection (unused here, kept for API compatibility).
            routing_info: Optional tuple of (routing_probs, routing_map) for health monitoring.
        """
        ...
        # Expert health monitoring: compute per-expert activation norms
        if routing_info is not None and torch.is_grad_enabled():
            self._track_expert_health(expert_output, tokens_per_expert, routing_info)

        output = self.token_dispatcher.combine_preprocess(expert_output)

        return output, mlp_bias

    def _track_expert_health(
        self,
        expert_output: torch.Tensor,
        tokens_per_expert: torch.Tensor,
        routing_info: tuple,
    ):
        """Track expert health metrics for monitoring.

        Computes per-expert activation norms and saves them to the health tracker.
        This helps detect:
        - Expert collapse (min-to-median ratio drops)
        - Localized activation blow-up (max-to-median ratio explodes)

        Args:
            expert_output: Output tensor from experts, shape [total_tokens, hidden_size]
            tokens_per_expert: Number of tokens per expert, shape [num_local_experts]
            routing_info: Tuple of (routing_probs, routing_map) from router.
        """
        routing_probs, routing_map = routing_info

        with torch.no_grad():
            # Convert tokens_per_expert to list if it's a tensor
            if torch.is_tensor(tokens_per_expert):
                tpe_list = tokens_per_expert.tolist()
            else:
                tpe_list = list(tokens_per_expert)

            # Compute per-expert RMS activation norm
            expert_norms = []
            start_idx = 0
            for num_tokens in tpe_list:
                if num_tokens > 0:
                    expert_tokens = expert_output[start_idx : start_idx + num_tokens]
                    # RMS norm: sqrt(mean(x^2))
                    rms_norm = torch.sqrt(torch.mean(expert_tokens.float() ** 2))
                    expert_norms.append(rms_norm)
                else:
                    # No tokens for this expert, use 0
                    expert_norms.append(torch.tensor(0.0, device=expert_output.device))
                start_idx += num_tokens

            if expert_norms:
                expert_activation_norms = torch.stack(expert_norms)
                save_to_expert_health_tracker(
                    layer_number=self.layer_number,
                    num_layers=self.config.num_layers,
                    expert_activation_norms=expert_activation_norms,
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        def custom_forward(hidden_states):
            ...
            hidden_states, probs, residual, routing_info = self.router_and_preprocess(hidden_states)
