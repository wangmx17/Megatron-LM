...
class Attention(MegatronModule, ABC):
    ...
    def forward(
        ...
        *,
        ...
        # Get gate_score if elementwise_attn_output_gate is enabled
        gate_score = getattr(self, '_gate_score', None)
        ...
        if in_decode_mode and self.config.flash_decode:
            ...
            # Apply elementwise gate if gate_score is provided
            if gate_score is not None:
                # gate_score shape: [sq, b, np, hn] -> [sq, b, np*hn]
                gate_score_flat = gate_score.view(gate_score.size(0), gate_score.size(1), -1)
                context_layer = context_layer * torch.sigmoid(gate_score_flat)
        ...
        # ================================================
        # NSA: save pre-RoPE query/key for stage1 NOPE
        # ================================================
        from megatron.core.transformer.nsa_attention import NSADotProductAttention, NSAHybridDotProductAttention
        is_nsa_core = isinstance(self.core_attention, (NSADotProductAttention, NSAHybridDotProductAttention))

        if is_nsa_core and self.core_attention.nsa_stage1_nope:
            query_nope = query.clone()
            key_nope = key.clone()  # save before applying RoPE, for NSA stage1 NOPE
        else:
            query_nope = None
            key_nope = None
        ...
        if self.checkpoint_core_attention and self.training:
            ...
            # NSA ignores gate_score internally; apply it here (TE handles gate on its own)
            if is_nsa_core and gate_score is not None:
                core_attn_out = core_attn_out * torch.sigmoid(gate_score.squeeze(1))
        else:
            if inference_context is None or inference_context.is_static_batching():
                # Static batching attention kernel.
                if is_nsa_core:
                    # NSA needs query_nope and key_nope for stage1 NOPE
                    ...
                    # NSA doesn't apply gate internally; apply elementwise gate here
                    if gate_score is not None:
                        core_attn_out = core_attn_out * torch.sigmoid(gate_score.squeeze(1))
                else:
                    core_attn_out = self.core_attention(
                ...
                # Apply elementwise gate if gate_score is provided
                # Note: flash_decode_and_prefill returns [b, s, h*d], but we need [s, b, h*d]
                # gate_score is [sq, b, np, hn], need to reshape to [sq, b, np*hn]
                if gate_score is not None:
                    gate_score_flat = gate_score.view(gate_score.size(0), gate_score.size(1), -1)
                    # core_attn_out is [b, s, h*d], transpose to [s, b, h*d] to match gate_score
                    core_attn_out = core_attn_out.transpose(0, 1)
                    core_attn_out = core_attn_out * torch.sigmoid(gate_score_flat)
                    core_attn_out = core_attn_out.transpose(0, 1)
                # Return as tuple to match expected format
                core_attn_out = (core_attn_out, None)
...
class SelfAttention(Attention):
    ...
    def __init__(
        ...
        # Check if elementwise_attn_output_gate is enabled
        self.elementwise_attn_output_gate = getattr(self.config, 'elementwise_attn_output_gate', True)
        
        # If elementwise_attn_output_gate is enabled, we need to double the query projection size
        # to include gate_score: query_projection_size * 2
        query_size = self.query_projection_size
        if self.elementwise_attn_output_gate:
            query_size = self.query_projection_size * 2

        self.linear_qkv = build_module(
            submodules.linear_qkv,
            self.config.hidden_size,
            query_size + 2 * self.kv_projection_size,
            config=self.config,
    ...
    def get_query_key_value_tensors(self, hidden_states, key_value_states=None, split_qkv=True):
        ...
        If elementwise_attn_output_gate is enabled, also extracts gate_score from query.
        """
        # Attention heads [sq, b, h] --> [sq, b, ng * (np/ng + 2) * hn)]
        # Attention heads [sq, b, h] --> [sq, b, ng * (np/ng + 2) * hn)] or with gate: [sq, b, ng * (np/ng * 2 + 2) * hn]
        ...
        if self.elementwise_attn_output_gate:
            query_dim_per_group = query_dim_per_group * 2

        # [sq, b, hp] --> [sq, b, ng, (np/ng * (1 or 2) + 2) * hn]
        new_tensor_shape = mixed_qkv.size()[:-1] + (
            ...
            #     * self.hidden_size_per_attention_head
            # ),
            query_dim_per_group + 2 * self.hidden_size_per_attention_head,
        )
        ...
        if SplitAlongDim is not None:
            ...
            # [sq, b, ng, (np/ng * (1 or 2) + 2) * hn]
            # --> [sq, b, ng, np/ng * (1 or 2) * hn], [sq, b, ng, hn], [sq, b, ng, hn]
            (query_with_gate, key, value) = SplitAlongDim(mixed_qkv, 3, split_arg_list)
        else:
            ...
            # [sq, b, ng, (np/ng * (1 or 2) + 2) * hn]
            # --> [sq, b, ng, np/ng * (1 or 2) * hn], [sq, b, ng, hn], [sq, b, ng, hn]
            (query_with_gate, key, value) = torch.split(mixed_qkv, split_arg_list, dim=3)

        # Extract gate_score if elementwise_attn_output_gate is enabled
        gate_score = None
        if self.elementwise_attn_output_gate:
            # Split query_with_gate into query and gate_score
            # [sq, b, ng, np/ng * 2 * hn] -> [sq, b, ng, np/ng * hn], [sq, b, ng, np/ng * hn]
            query_dim = (
                self.num_attention_heads_per_partition
                // self.num_query_groups_per_partition
                * self.hidden_size_per_attention_head
            )
            query, gate_score = torch.split(query_with_gate, [query_dim, query_dim], dim=3)
            # [sq, b, ng, np/ng * hn] -> [sq, b, np, hn]
            gate_score = gate_score.reshape(
                gate_score.size(0), gate_score.size(1), -1, self.hidden_size_per_attention_head
            )
        else:
            query = query_with_gate
        ...
        # Store gate_score as instance variable for use in forward
        self._gate_score = gate_score
