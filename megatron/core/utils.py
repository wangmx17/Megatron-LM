...
def split_tensor_evenly(tensor, split_dim, cu_seqlen, N, r_list):
    span_sizes = cu_seqlen[1:] - cu_seqlen[:-1]
    chunk_sizes = span_sizes // N  # Size of each chunk within a span
    indices_list = [] 
    # print(f"split evenly {cu_seqlen} {chunk_sizes}")
    for r in r_list:
        start_indices = cu_seqlen[:-1] + r * chunk_sizes
        end_indices = cu_seqlen[:-1] + (r + 1) * chunk_sizes
        indices = torch.cat(
            [torch.arange(start, end, device="cuda") for start, end in zip(start_indices, end_indices)]
        )
        # print(f"split evenly {start_indices} {end_indices} {indices.shape}")
        indices_list.append(indices)
    indices_list = torch.cat(indices_list, dim=0)
    chunks = torch.index_select(tensor, split_dim, indices_list)
...
def reorder_cp_chunk_seq_order(tensor: torch.Tensor, seq_dim: int, num_chunks: int):
    '''
    This serves for cp with / without span based attention.
    Params:
        - tensor: the tensor to reorder
        - seq_dim: the dimension to reorder
        - num_chunks: context parallel size
    '''
    ori_shape = list(tensor.shape)
    total_tokens = tensor.shape[seq_dim]
    assert (
        total_tokens % (2 * num_chunks) == 0
    ), "Sequence length must be divisible by 2*num_chunks for CP reordering."

    chunk_len = total_tokens // (2 * num_chunks)

    # View as [ ..., num_chunks, 2, chunk_len, ... ] along seq_dim
    view_shape_prefix = list(tensor.shape[:seq_dim])
    view_shape_suffix = list(tensor.shape[(seq_dim + 1) :])
    ...
    # Construct pieces in natural sequential order of chunk indices
    # First half: (rank=r, h=0) for r=0..num_chunks-1
    pieces = [
        tensor_view.select(seq_dim, r).select(seq_dim, 0) for r in range(num_chunks)
    ]
    # Second half: (rank=r, h=1) for r=num_chunks-1..0 (reverse ranks)
    pieces += [
        tensor_view.select(seq_dim, r).select(seq_dim, 1)
        for r in range(num_chunks - 1, -1, -1)
    ]

    reordered = torch.cat(pieces, dim=seq_dim)
    assert list(reordered.shape) == ori_shape
    return reordered

def update_packer_params_cp_with_span_based_attn(params: PackedSeqParams, world_size, rank):
    '''
    This function is used to update the packed_seq_params for context parallel.
    We assume that cp takes two chunks per rank, one from the head and one from the tail (according to the implementation in get_batch function)

    Now we DO NOT reorder the gathered tensor, instead directly select the indices.
    '''
    # TODO typically we assume that seq_dim = 0, or this func may not work
    cu_seqlens = params.cu_seqlens_q
    tot_seqlen = cu_seqlens[-1]
    rank_tot_seqlen = tot_seqlen // world_size

    # 每个cu_seqlen_q = rank所有q的一半
    cu_seqlen_q = cu_seqlens // (2 * world_size) # (seq_len, ...)
    half_seqlen_q = cu_seqlen_q[1:] - cu_seqlen_q[:-1]
    cu_seqlen_kv_head = cu_seqlen_q * (rank + 1)
    cu_seqlen_kv_tail = cu_seqlen_q * (2 * world_size - rank - 1 + 1)

    # 每个rank的所有kv内部的varlen关系
    cu_seqlen_kv = cu_seqlens // world_size 
    kv_head_indices = []
    kv_tail_indices = []

    for idx, block_size in enumerate(half_seqlen_q):
        # collect needed head
        for i in range(rank + 1): # [0, rank] 
            this_head_indices = rank_tot_seqlen * i + torch.arange(cu_seqlen_kv[idx], cu_seqlen_kv[idx] + block_size, device=cu_seqlen_q.device)
            kv_head_indices.append(this_head_indices)
        # collect all head for tail
        for i in range(world_size): # [0, world_size - 1]
            this_tail_indices = rank_tot_seqlen * i + torch.arange(cu_seqlen_kv[idx], cu_seqlen_kv[idx] + block_size, device=cu_seqlen_q.device)
            kv_tail_indices.append(this_tail_indices)
        # collect needed tail
        for i in range(world_size - 1, rank - 1, -1): # [world_size - 1, rank], reverse order
            this_tail_indices = rank_tot_seqlen * i + torch.arange(cu_seqlen_kv[idx] + block_size, cu_seqlen_kv[idx] + 2 * block_size, device=cu_seqlen_q.device)
            kv_tail_indices.append(this_tail_indices)
    
    kv_head_indices = torch.cat(kv_head_indices, dim=0)
    kv_tail_indices = torch.cat(kv_tail_indices, dim=0)

    gathered_kv_len = tot_seqlen
    if kv_head_indices.numel() > 0 and kv_head_indices.max() >= gathered_kv_len:
        raise IndexError(
            f"[CP span attn] kv_head_indices OOB: max_idx={kv_head_indices.max().item()}, "
            f"gathered_kv_len={gathered_kv_len}, rank={rank}, world_size={world_size}, "
            f"cu_seqlens={cu_seqlens.tolist()}"
        )
    if kv_tail_indices.numel() > 0 and kv_tail_indices.max() >= gathered_kv_len:
        raise IndexError(
            f"[CP span attn] kv_tail_indices OOB: max_idx={kv_tail_indices.max().item()}, "
            f"gathered_kv_len={gathered_kv_len}, rank={rank}, world_size={world_size}, "
            f"cu_seqlens={cu_seqlens.tolist()}"
        )

    def max_span(_cu_seqlen):
        return (_cu_seqlen[1:] - _cu_seqlen[:-1]).max().item()

    params_head = PackedSeqParams(
        qkv_format="thd",
        cu_seqlens_q=cu_seqlen_q,
        cu_seqlens_kv=cu_seqlen_kv_head,
        max_seqlen_q=max_span(cu_seqlen_q),
        max_seqlen_kv=max_span(cu_seqlen_kv_head),
    )
    params_tail = PackedSeqParams(
        qkv_format="thd",
        cu_seqlens_q=cu_seqlen_q,
        cu_seqlens_kv=cu_seqlen_kv_tail,
        max_seqlen_q=max_span(cu_seqlen_q),
        max_seqlen_kv=max_span(cu_seqlen_kv_tail),
    )

    return params_head, params_tail, kv_head_indices, kv_tail_indices


def update_packer_params_cp_without_span_based_attn(params: PackedSeqParams, world_size, rank):
    '''
    This function is used to update the packed_seq_params for context parallel.
    !!! DO NOT support span based attention !!!
    We assume that cp takes two chunks per rank, one from the head and one from the tail (according to the implementation in get_batch function)
    '''
    max_seqlen = params.max_seqlen_q
    block_nums = world_size * 2
    if max_seqlen % block_nums != 0:
        raise ValueError(
            f"[CP no-span] max_seqlen={max_seqlen} not divisible by "
            f"2*cp_size={block_nums}. Check data pipeline padding."
        )
    block_size = max_seqlen // block_nums
    block_indexes = [rank, block_nums - rank - 1]

    cu_seqlens_q_head = torch.tensor([0, block_size], dtype=params.cu_seqlens_q.dtype, device=params.cu_seqlens_q.device)
    cu_seqlens_q_tail = torch.tensor([0, block_size], dtype=params.cu_seqlens_q.dtype, device=params.cu_seqlens_q.device)
    cu_seqlens_kv_head = torch.tensor([0, block_size * (block_indexes[0] + 1)], dtype=params.cu_seqlens_kv.dtype, device=params.cu_seqlens_kv.device)
    cu_seqlens_kv_tail = torch.tensor([0, block_size * (block_indexes[1] + 1)], dtype=params.cu_seqlens_kv.dtype, device=params.cu_seqlens_kv.device)
    
    max_seqlen_q_head = block_size
    max_seqlen_q_tail = block_size
    max_seqlen_kv_head = block_size * (block_indexes[0] + 1)
    max_seqlen_kv_tail = block_size * (block_indexes[1] + 1)
    return PackedSeqParams(
        qkv_format="thd",
        cu_seqlens_q=cu_seqlens_q_head,
        cu_seqlens_kv=cu_seqlens_kv_head,
        max_seqlen_q=max_seqlen_q_head,
        max_seqlen_kv=max_seqlen_kv_head,
    ), PackedSeqParams(
        qkv_format="thd",
        cu_seqlens_q=cu_seqlens_q_tail,
        cu_seqlens_kv=cu_seqlens_kv_tail,
        max_seqlen_q=max_seqlen_q_tail,
        max_seqlen_kv=max_seqlen_kv_tail,
    )

def get_query_list_indices(params: PackedSeqParams, world_size):
    # use_span状态下每个span里面前一半后一半各是head query和tail query
    # rank相关的已经分好了，这个函数只是把head query和tail query分开返回
    cu_seqlens_q = params.cu_seqlens_q
    cp_cu_seqlens_q = cu_seqlens_q // world_size
    seq_lens = cu_seqlens_q[1:] - cu_seqlens_q[:-1]
    block_sizes = seq_lens // (2 * world_size)

    head_indices, tail_indices = [], []
    for idx, block_size in enumerate(block_sizes):
        head_start = cp_cu_seqlens_q[idx]
        head_end = head_start + block_size
        tail_start = head_end
        tail_end = cp_cu_seqlens_q[idx + 1]
        head_indices.append(torch.arange(head_start, head_end, device=cu_seqlens_q.device))
        tail_indices.append(torch.arange(tail_start, tail_end, device=cu_seqlens_q.device))

    head_indices = torch.cat(head_indices, dim=0)
    tail_indices = torch.cat(tail_indices, dim=0)
...
def get_batch_on_this_cp_rank(batch: Dict[str, Any]):
    ...
    batch_size, max_seqlen = batch['position_ids'].shape
    cu_seqlens = torch.nonzero(batch['position_ids'].flatten() == 0).flatten()
    cu_seqlens = torch.nn.functional.pad(cu_seqlens, (0, 1), 'constant', batch_size * max_seqlen).type(torch.int32)
    args.current_cu_seqlens = cu_seqlens.tolist()

    # Accumulate sum(s_i^2) across micro-batches for accurate TFLOPS calculation.
    span_lens = (cu_seqlens[1:] - cu_seqlens[:-1]).float()
    sum_sq = (span_lens * span_lens).sum().item()
    if not hasattr(args, '_attn_span_sq_sum'):
        args._attn_span_sq_sum = 0.0
        args._attn_span_micro_batches = 0
    args._attn_span_sq_sum += sum_sq
    args._attn_span_micro_batches += 1

    if cp_size > 1 and args.use_span_based_attn:
        ...
        span_sizes = cu_seqlens[1:] - cu_seqlens[:-1]
        bad_spans = span_sizes % (2 * cp_size) != 0
        if bad_spans.any():
            dataset_id = batch.get('dataset_id', None)
            pos_ids = batch['position_ids'].flatten()
            bad_idx = torch.nonzero(bad_spans).flatten().tolist()[:10]
            bad_details = [(int(cu_seqlens[i]), int(cu_seqlens[i+1]), int(span_sizes[i])) for i in bad_idx]
            raise ValueError(
                f"[CP+THD] Span lengths must be divisible by 2*cp_size={2 * cp_size}, "
                f"bad {int(bad_spans.sum())}/{len(span_sizes)} spans. "
                f"dataset_id={dataset_id.tolist() if dataset_id is not None else 'N/A'}, "
                f"first bad (start,end,len): {bad_details}, "
                f"position_ids[:32]={pos_ids[:32].tolist()}, "
                f"position_ids zeros={int((pos_ids == 0).sum())}, "
                f"cu_seqlens[:30]={cu_seqlens[:30].tolist()}"
            )

        index = tex.thd_get_partitioned_indices(cu_seqlens, batch_size * max_seqlen, cp_size, cp_rank)
        for key, val in batch.items():
            ...
            if val is not None:
                ...
                if seq_dim == 1:
                    ...
                    val = split_tensor_evenly(val, seq_dim, cu_seqlens, 2*cp_size, [cp_rank, 2*cp_size - cp_rank - 1])
    ...
    if args.use_span_based_attn:
        ...
        if cp_size > 1:
            cp_rank = parallel_state.get_context_parallel_rank()
            batch['packed_seq_params'].cp_packed_params_w_span_attn = update_packer_params_cp_with_span_based_attn(batch['packed_seq_params'], cp_size, cp_rank)
            batch['packed_seq_params'].cp_query_indices_w_span_attn = get_query_list_indices(batch['packed_seq_params'], cp_size)
    elif getattr(args, 'use_nsa', False):
        total_length = batch_size * max_seqlen
        _cu_seqlens = torch.arange(
            0, total_length + 1, max_seqlen,
            dtype=torch.int32, device=batch['position_ids'].device,
        )
        batch['packed_seq_params'] = PackedSeqParams(
            qkv_format="thd",
            cu_seqlens_q=_cu_seqlens,
            cu_seqlens_kv=_cu_seqlens,
            max_seqlen_q=max_seqlen,
            max_seqlen_kv=max_seqlen,
        )
        if cp_size > 1:
            cp_rank = parallel_state.get_context_parallel_rank()
            batch['packed_seq_params'].cp_packed_params_wo_span_attn = update_packer_params_cp_without_span_based_attn(batch['packed_seq_params'], cp_size, cp_rank)
    else:
