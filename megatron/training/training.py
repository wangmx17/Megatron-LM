...
from megatron.core.transformer.moe.moe_utils import (
    ...
    clear_expert_health_tracker_for_new_step,
)
...
def num_floating_point_operations(args, batch_size):
    # The attention quadratic term (QK^T, attn*V) depends on actual span lengths.
    # Use accumulated sum(s_i^2) across all micro-batches for exact calculation.
    # This works for both span-based and non-span-based attention:
    #   - With spanbase: sum(s_i^2) < micro_bs * seq_length^2 (multiple short spans)
    #   - Without spanbase: sum(s_i^2) = micro_bs * seq_length^2 (one full-length span)
    attn_span_sq_sum = getattr(args, '_attn_span_sq_sum', None)
    if attn_span_sq_sum is not None and attn_span_sq_sum > 0:
        num_micro_batches = args._attn_span_micro_batches
        total_samples = num_micro_batches * args.micro_batch_size
        attn_seq_factor = attn_span_sq_sum / (total_samples * args.seq_length)
    else:
        attn_seq_factor = args.seq_length
    ...
    def attn_layer_flops(
        ...
        Uses attn_seq_factor (from closure) for the attention quadratic term,
        which accounts for span-based attention packing when enabled.
        """
        ...
        return (
            ...
            * hidden_size
            * p
            * (hidden_size + (hidden_size * (g / num_heads)) + (attn_seq_factor / 2))
        )
            ...
            self_attn_term = (
                ...
                * (
                    ...
                    * (
                    ...
                    ## core attn (attn_seq_factor accounts for span-based attention packing)
                    + attn_seq_factor
                    * (args.num_attention_heads * (args.qk_head_dim + args.qk_pos_emb_head_dim))
                    / 2
                    + attn_seq_factor * args.num_attention_heads * args.v_head_dim / 2
                )
            )
            ...
            self_attn_term = (
                ...
                * (
                    (
                        1
                        + (args.num_query_groups / args.num_attention_heads)
                        # Only half of the attention matrix is non-zero (causal).
                        # attn_seq_factor accounts for span-based attention packing.
                        + (attn_seq_factor / args.hidden_size / 2)
                    )
    ...
    if not args.skip_train:
        ...
        if args.save and iteration != 0 and iteration % args.save_interval != 0:
            if args.use_modelbest_sdk:
                save_dataset_checkpoint(train_data_iterator, iteration)
            save_checkpoint(
                iteration,
                model,
                optimizer,
...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config, forward_backward_func):
    ...
    # Clear expert health tracker at the start of each step to ensure per-step metrics
    if args.num_experts is not None and args.moe_expert_health_monitoring:
        clear_expert_health_tracker_for_new_step()
    ...
    if mpu.is_pipeline_last_stage(ignore_virtual=True):
        ...
        if "task_tokens_list" in losses_reduced[0] and "task_losses_list" in losses_reduced[0]:
            val_tokens = torch.vstack([x["task_tokens_list"] for x in losses_reduced]).sum(dim=0)
            val_losses = torch.vstack([x["task_losses_list"] for x in losses_reduced]).sum(dim=0)
            ...
            loss_reduced = {"lm_loss/" + "/".join(config.dataset_path_list[key].strip("/").split("/")[-2:]): val_losses[key] / val_tokens[key] for key in torch.nonzero(val_tokens)}


            if "task_seq_count_list" in losses_reduced[0] and "task_token_count_list" in losses_reduced[0]:
                task_seq_counts = torch.vstack([x["task_seq_count_list"] for x in losses_reduced]).sum(dim=0)
                task_token_counts = torch.vstack([x["task_token_count_list"] for x in losses_reduced]).sum(dim=0)
                torch.distributed.all_reduce(
                    task_seq_counts,
                    group=mpu.get_data_parallel_group(with_context_parallel=True)
                )
                ...
                for key_item in torch.nonzero(task_seq_counts):
                    avg_seq_len = (task_token_counts[key_item] / task_seq_counts[key_item]).detach().item()
                    dataset_name = "/".join(config.dataset_path_list[key_item].strip("/").split("/")[-2:])
                    loss_reduced[f'avg_seq_len/{dataset_name}'] = avg_seq_len

                total_seq_count = task_seq_counts.sum()
                total_token_count = task_token_counts.sum()
                if total_seq_count > 0:
                    global_avg_seq_len = (total_token_count / total_seq_count).detach().item()
                    loss_reduced['avg_seq_len/global'] = global_avg_seq_len

            # Per-dataset think / answer loss split. Mirrors the `lm_loss/<ds>`
            # reduction above; produces TB tags `lm_loss_think/<ds>` and
            # `lm_loss_answer/<ds>` (sorts adjacent to `lm_loss/<ds>` in TB).
            for split_name, loss_key, tokens_key in (
                ("lm_loss_think", "task_think_losses_list", "task_think_tokens_list"),
                ("lm_loss_answer", "task_answer_losses_list", "task_answer_tokens_list"),
            ):
                if loss_key not in losses_reduced[0] or tokens_key not in losses_reduced[0]:
                    continue
                split_tokens = torch.vstack([x[tokens_key] for x in losses_reduced]).sum(dim=0)
                split_losses = torch.vstack([x[loss_key] for x in losses_reduced]).sum(dim=0)
                torch.distributed.all_reduce(
                    split_tokens,
                    group=mpu.get_data_parallel_group(with_context_parallel=True)
                )
                ...
                for key_item in torch.nonzero(split_tokens):
                    dataset_name = "/".join(config.dataset_path_list[key_item].strip("/").split("/")[-2:])
                    loss_reduced[f"{split_name}/{dataset_name}"] = split_losses[key_item] / split_tokens[key_item]
        else:
            loss_reduced = {}

        scalar_loss_keys = {"lm loss", "lm loss think", "lm loss answer"}
        for key in losses_reduced[0].keys():
            if key not in scalar_loss_keys:
                continue
            ...
            if val[0].numel() == 2:
                if args.sft:
                    ...
                    # Guard against micro-batches with zero tokens in the bucket
                    # (e.g. a sample without any <think> span when reporting
                    # `think loss`).
        ...
        if 'packing-efficiency' in losses_reduced[0]:
            pe_per_mb = torch.stack([x['packing-efficiency'].view(-1) for x in losses_reduced])
            pe_mb_ratios = pe_per_mb[:, 0] / pe_per_mb[:, 1]
            loss_reduced['packing/micro-batch-min'] = pe_mb_ratios.min()
            loss_reduced['packing/micro-batch-max'] = pe_mb_ratios.max()
            pe_global = pe_per_mb.sum(dim=0)
            torch.distributed.all_reduce(
                pe_global, group=mpu.get_data_parallel_group(with_context_parallel=True)
            )
            loss_reduced['packing/global-batch'] = pe_global[0] / pe_global[1]
...
def training_log(
    ...
    if args.timing_log_level >= 2:
        timers_to_log.extend([
            'batch-generator',
            'forward-compute',
            # 'forward-compute-set-input',
            # 'forward-compute-model-forward',
            # 'forward-compute-loss-calc',
            'backward-compute',
            'forward-recv',
            'forward-send',
            'backward-recv',
            ...
            # HybridEP dispatch/combine（等价于 all-to-all），timing-log-option=all 时打印各 rank
            # 'HybridEP_dispatch_fwd',
            # 'HybridEP_combine_fwd',
            # 'HybridEP_combine_bwd',
            # 'HybridEP_dispatch_bwd',
        ])
    ...
    # Aggregate drop stats across DP group (all ranks must participate).
    _drop_stats_reduced = None
    if hasattr(args, '_drop_total_samples') and (iteration % args.log_interval == 0):
        _drop_stats_reduced = torch.tensor(
            [args._drop_total_samples, args._drop_total_tokens],
            dtype=torch.float, device='cuda',
        )
        torch.distributed.all_reduce(
            _drop_stats_reduced, group=mpu.get_data_parallel_group(with_context_parallel=True),
        )
    ...
    if args.num_experts is not None:
        ...
        # Track expert health metrics (activation norms, routing confidence, etc.)
        # Logging cadence follows --log-task-loss-interval; gating in arguments.py
        # ensures monitoring is disabled when log_task_loss_interval == -1.
        if (
            args.moe_expert_health_monitoring
            and args.log_task_loss_interval > 0
            and iteration % args.log_task_loss_interval == 0
        ):
            track_expert_health_metrics(
                iteration=iteration,
                writer=writer,
    ...
    # hidden rms, max attention logit, logger
    StatesLoggingHelper.track_states_metrics(iteration, writer, wandb_writer, args.log_hidden_rms_interval, args.log_max_attention_logit_interval)

    # Per-step Prometheus timing update (runs every iteration, independent of log_interval)
    from megatron.training.prometheus_exporter import get_metrics_store, update_iteration_timings
    if get_metrics_store() is not None:
        update_iteration_timings(
            iteration=iteration,
            timers=timers,
    ...
    if iteration % args.log_interval == 0:
        ...
        if _drop_stats_reduced is not None:
            if writer:
                writer.add_scalar('drop/total-samples', _drop_stats_reduced[0].item(), iteration)
                writer.add_scalar('drop/total-tokens', _drop_stats_reduced[1].item(), iteration)
            if wandb_writer:
                wandb_writer.log({
                    'drop/total-samples': _drop_stats_reduced[0].item(),
                    'drop/total-tokens': _drop_stats_reduced[1].item(),
                }, iteration)

        # All-reduce attention span stats across DP ranks for accurate TFLOPS.
        # Reduce across DP+CP group, then divide by cp_size because all CP ranks
        # within the same DP group computed cu_seqlens from the same full batch
        # (before CP split), so their accumulated values are identical.
        if hasattr(args, '_attn_span_sq_sum') and args._attn_span_sq_sum > 0:
            span_stats = torch.tensor(
                [args._attn_span_sq_sum, args._attn_span_micro_batches],
                dtype=torch.float64, device='cuda'
            )
            dp_group = mpu.get_data_parallel_group(with_context_parallel=True)
            torch.distributed.all_reduce(span_stats, op=torch.distributed.ReduceOp.SUM, group=dp_group)
            cp_size = args.context_parallel_size
            args._attn_span_sq_sum = span_stats[0].item() / cp_size
            args._attn_span_micro_batches = int(span_stats[1].item()) // cp_size

        throughput = num_floating_point_operations(args, batch_size) / (
            elapsed_time_per_iteration * 10**12 * args.world_size
        )

        # Reset span-based attention accumulator after TFLOPS calculation
        if hasattr(args, '_attn_span_sq_sum'):
        ...
        if _drop_stats_reduced is not None:
            log_string += f' drop/total-samples: {int(_drop_stats_reduced[0].item())} |'
            log_string += f' drop/total-tokens: {int(_drop_stats_reduced[1].item())} |'
...
def save_dataset_checkpoint(dataloader, iteration):
    timers = get_timers()
    # Extra barrier is added to make sure all ranks report the max time.
    timers('save-dataset-checkpoint', log_level=0).start(barrier=True)
    directory = 'iter_{:07d}'.format(iteration)
    dataloader.save(directory)
    timers('save-dataset-checkpoint').stop(barrier=True)
    timers.log(['save-dataset-checkpoint'])
...
def train(
    ...
    if not getattr(args, 'no_prometheus', False) and getattr(args, 'prometheus_port', None) is not None and args.timing_log_level >= 1:
        from megatron.training import prometheus_exporter
        rank = torch.distributed.get_rank()
        local_rank = torch.cuda.current_device()
        try:
            prometheus_exporter.start_server(args.prometheus_port, rank, local_rank)
            if local_rank == 0:
                print(
                    f'> Started Prometheus metrics server on port {args.prometheus_port} '
                    f'(aggregating all local GPUs)',
                    flush=True,
                )
        except OSError as e:
            if local_rank == 0:
                print(
                    f'> WARNING: Failed to start Prometheus server on port '
                    f'{args.prometheus_port}: {e}',
                    flush=True,
                )
...
def build_train_valid_test_data_loaders(build_train_valid_test_datasets_provider):
    ...
    if is_distributed or mpu.get_tensor_model_parallel_rank() == 0:
        ...
        if getattr(args, 'perform_rl_step', True):
            ...
            do_valid = valid_dataloaders != [None] and (args.full_validation or args.eval_iters > 0)
