...
def add_megatron_arguments(parser: argparse.ArgumentParser):
    ...
    parser = _add_prometheus_metrics_args(parser)
...
def validate_args(args, defaults={}):
    ...
    if getattr(args, 'log_think_loss', False) and args.log_task_loss_interval == -1:
        warn_rank_0(
            "--log-think-loss is enabled but --log-task-loss-interval is -1; "
            "think/answer loss split will be disabled. Set --log-task-loss-interval "
            "to a positive value (e.g. 100) to enable it.",
            args.rank,
        )
        args.log_think_loss = False

    if getattr(args, 'moe_expert_health_monitoring', False) and args.log_task_loss_interval == -1:
        warn_rank_0(
            "--moe-expert-health-monitoring is enabled but --log-task-loss-interval is -1; "
            "expert health metrics will not be logged. Set --log-task-loss-interval "
            "to a positive value (e.g. 100) to enable logging.",
            args.rank,
        )
        args.moe_expert_health_monitoring = False
...
def _add_prometheus_metrics_args(parser):
    group = parser.add_argument_group(title='prometheus metrics')
    group.add_argument('--prometheus-port', type=int, default=9090,
                       help='Port for the Prometheus HTTP server '
                       '(local_rank 0 only), aggregating iteration timing '
                       'metrics from ALL local GPUs at /metrics.  '
                       'Requires --timing-log-level >= 1.  The set of exported '
                       'timers follows --timing-log-level.  Default: 9090.')
    group.add_argument('--no-prometheus', action='store_true', default=False,
...
def _add_regularization_args(parser):
    ...
    group.add_argument('--muon-split-qkv', action='store_true', help='Whether to split QKV parameters for Muon optimizer')
    group.add_argument('--muon-coefficient-type', type=str, default="simple", choices=["simple", "quintic"])
...
def _add_training_args(parser):
    ...
    group.add_argument('--log-task-loss-interval', type=int, default=-1,
                       help='Report multi task loss and timing interval.')
    group.add_argument('--log-packing-efficiency', action='store_true', default=False,
                       help='Log packing efficiency (non-padding token ratio) to tensorboard.')
    group.add_argument('--track-drop-stats', action='store_true', default=False,
                       help='Track and log dropped samples/tokens stats to tensorboard.')
    group.add_argument('--log-think-loss', action='store_true', default=False,
                       help='Enable think/answer loss split monitoring '
                        '(requires --log-task-loss-interval > 0).')
    group.add_argument('--log-hidden-rms-interval', type=int, default=None,
                    help='Interval to report the root mean square of the hidden states.')
    group.add_argument('--log-max-attention-logit-interval', type=int, default=None,
                       help='Interval to report the max logit in attention matrix.')
    ...
    group.add_argument('--use-triton-rope', action='store_true',
                       help='Use Triton-based RoPE THD kernel instead of TE '
                       'fused kernel. Eliminates O(n_spans) grid explosion.')
    group.add_argument('--use-nsa', action='store_true',
                       help='Enable Native Sparse Attention')
    group.add_argument('--nsa-block-size', type=int, default=32,
                      help='Block size for sparse attention')
    group.add_argument('--nsa-sliding-window', type=int, default=0,
                      help='Sliding window size for local attention')
    group.add_argument('--nsa-sparse-blocks', type=int, default=8,
                      help='Number of sparse blocks per attention head')
    
    group.add_argument('--use-compress', action='store_true',
                       help='Enable compress of NSA')
    group.add_argument('--use-topk', action='store_true',
                       help='Enable topk of NSA')
    group.add_argument('--nsa-compress-func', type=str, default='meanpool',
                       help='compress-func of NSA')
    group.add_argument('--freeze-no-nsa-iters', type=int, default=0,
                       help='冻结非nsa参数')
    group.add_argument('--nsa-topk-num', type=int, default=32,
                       help='topk of NSA')
    
    group.add_argument('--nsa-disable-short', action='store_true',
                        help='Disable NSA for short examples.')
    group.add_argument('--nsa-disable-short-threshold', type=int, default=4096,
                       help='Threshold for disabling NSA for short examples.')
    group.add_argument('--nsa-stage1-nope', action='store_true',
                       help='Use NOPE (no positional embedding) Q/K for NSA stage1 top-k selection.')
...
def _add_data_args(parser):
    ...
    group.add_argument('--use-modelbest-sdk', action="store_true", 
                        help='Use modelbest sdk to load data.')
    group.add_argument('--clear-sampler-state', action='store_true', 
                       help='whether clear the max error state of megatron data sampler')
    group.add_argument('--no-load-data-state', action='store_true',
                       help='whether load dataset state when modelbest-sdk')
    ...
    group.add_argument('--data-chunk-size', type=int, default=64,
                       help="Number of samples per chunk in modelbest SDK dataloader.")
    group.add_argument('--prefetch-chunk-cnt', type=int, default=2,
                       help="Number of chunks to prefetch per dataset in modelbest SDK.")
...
def _add_moe_args(parser):
    ...
    group.add_argument('--moe-expert-health-monitoring', action='store_true',
                       help='Enable expert health monitoring for MoE. Tracks per-expert activation norms, '
                            'max-to-median ratio (detects localized activation blow-up), min-to-median ratio '
                            '(detects expert collapse), and routing confidence. '
                            'Logging cadence follows --log-task-loss-interval. '
