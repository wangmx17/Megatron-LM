...
from megatron.training import get_args, get_timers, get_adlr_autoresume, get_tokenizer
...
def get_batch_on_this_tp_rank(data_iterator):
    ...
    args = get_args()
    metadata_enabled = args.use_modelbest_sdk and args.log_task_loss_interval != -1
    num_real_seqs = None
    total_content_len = None
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        if args.use_modelbest_sdk:
            ...
            if metadata_enabled:
                num_real_seqs = data.get('num_real_seqs')
                total_content_len = data.get('total_content_len')
                if num_real_seqs is None:
                    num_real_seqs = torch.zeros(args.micro_batch_size, dtype=torch.int64)
                if total_content_len is None:
                    total_content_len = torch.zeros(args.micro_batch_size, dtype=torch.int64)
                num_real_seqs = num_real_seqs.to(torch.int64).cuda(non_blocking=True)
                total_content_len = total_content_len.to(torch.int64).cuda(non_blocking=True)
            if getattr(args, 'track_drop_stats', False):
                drop_stats = data.get('drop_stats')
                if drop_stats is None:
                    drop_stats = torch.zeros(2, dtype=torch.int64)
                drop_stats = drop_stats.to(torch.int64).cuda(non_blocking=True)
        ...
        batch = {
            ...
            'attention_mask': (
                None
                if ("attention_mask" not in data or args.create_attention_mask_in_dataloader == False)
        ...
        if args.pipeline_model_parallel_size == 1:
            ...
            _broadcast(drop_stats)
            if args.log_task_loss_interval != -1:
                _broadcast(batch['dataset_id'])
                if metadata_enabled:
                    _broadcast(num_real_seqs)
                    _broadcast(total_content_len)
        ...
        if metadata_enabled:
            num_real_seqs = torch.empty(
                (args.micro_batch_size,), dtype=torch.int64, device=torch.cuda.current_device()
            )
            total_content_len = torch.empty(
                (args.micro_batch_size,), dtype=torch.int64, device=torch.cuda.current_device()
            )
        if args.use_modelbest_sdk and getattr(args, 'track_drop_stats', False):
            drop_stats = torch.empty((2,), dtype=torch.int64, device=torch.cuda.current_device())
        else:
            drop_stats = None
    ...
    if metadata_enabled and (args.pipeline_model_parallel_size == 1 or mpu.is_pipeline_last_stage()):
        ...
        args.current_total_content_len = total_content_len
    else:
        args.current_num_real_seqs = None
        args.current_total_content_len = None

    if drop_stats is not None:
        if not hasattr(args, '_drop_total_samples'):
            args._drop_total_samples = 0
            args._drop_total_tokens = 0
        args._drop_total_samples += drop_stats[0].item()
        args._drop_total_tokens += drop_stats[1].item()

    return batch


_THINK_TAG_TOKEN_IDS = None


def _tokenize_text_to_ids(tokenizer, text):
    """Encode `text` to a list of token ids without auto-prepended BOS / specials.

    Megatron's tokenizer wrappers expose different APIs:
      * `_HuggingFaceTokenizer.tokenize(text, **kwargs)` forwards kwargs to the
        underlying HF `AutoTokenizer.__call__`, which by default sets
        `add_special_tokens=True` and would prepend BOS, breaking single-token
        detection. We pass `add_special_tokens=False` when supported.
      * Other wrappers (SentencePiece, GPT2BPE, ...) accept no kwargs but also
        don't add BOS in `tokenize()`, so a plain call is fine.

    As a final safety net we also strip whatever prefix `tokenize("")` returns,
    which catches tokenizers that unconditionally add a BOS / leading marker.
    """
    try:
        ids = tokenizer.tokenize(text, add_special_tokens=False)
    except TypeError:
        ids = tokenizer.tokenize(text)
        try:
            prefix = list(tokenizer.tokenize(""))
        except Exception:  # pylint: disable=broad-except
            prefix = []
        if prefix and len(ids) > len(prefix) and list(ids[:len(prefix)]) == prefix:
            ids = ids[len(prefix):]
    return list(ids)


def _resolve_think_tag_token_ids():
    """Resolve `<think>` / `</think>` token ids from the configured tokenizer.
    """
    global _THINK_TAG_TOKEN_IDS

    if _THINK_TAG_TOKEN_IDS is not None:
        return _THINK_TAG_TOKEN_IDS if _THINK_TAG_TOKEN_IDS is not False else None

    try:
        tokenizer = get_tokenizer()
        start_ids = _tokenize_text_to_ids(tokenizer, "<think>")
        end_ids = _tokenize_text_to_ids(tokenizer, "</think>")
    except Exception as e:  # pylint: disable=broad-except
        print_rank_0(
            "[think-loss] WARNING: failed to query tokenizer for <think>/</think> "
            f"ids ({e!r}); disabling --log-think-loss for this run."
        )
        _THINK_TAG_TOKEN_IDS = False
        return None

    if len(start_ids) != 1 or len(end_ids) != 1:
        print_rank_0(
            "[think-loss] WARNING: <think>/</think> are not single tokens "
            f"(got start_ids={start_ids}, end_ids={end_ids}); think/answer "
            "loss split would be unreliable, disabling --log-think-loss for "
            "this run."
        )
        _THINK_TAG_TOKEN_IDS = False
        return None

    _THINK_TAG_TOKEN_IDS = (start_ids[0], end_ids[0])
    print_rank_0(
        "[think-loss] resolved single-token tag ids: "
        f"<think>={_THINK_TAG_TOKEN_IDS[0]}, </think>={_THINK_TAG_TOKEN_IDS[1]}"
    )
    return _THINK_TAG_TOKEN_IDS


def should_log_think_loss(args):
    """True iff think/answer loss should be reported on this iteration.、
    """
    if not getattr(args, 'log_think_loss', False):
        return False
    interval = getattr(args, 'log_task_loss_interval', -1)
    if interval is None or interval == -1:
        return False
    # If tag-id resolution previously failed, short-circuit so we don't pay
    # the cost of building think_mask every step just to discard it.
    if _THINK_TAG_TOKEN_IDS is False:
        return False
    curr_iter = getattr(args, 'curr_iteration', 0)
    return (curr_iter + 1) % interval == 0


def build_think_mask(labels):
    """Per-token mask describing the `<think>...</think>` structure of each row.
        1.0 -> token is inside a <think>...</think> span (counts as think)
        0.0 -> token is outside a <think> span in a row that DOES contain
               <think> openers (counts as answer)
        2.0 -> entire row has no <think> opener (non-think dataset);
               excluded from BOTH lm_loss_think and lm_loss_answer.
    """
    if labels is None:
        return None
    ids = _resolve_think_tag_token_ids()
    if ids is None:
        return None
    start_id, end_id = ids
    start = (labels == start_id).int()
    end = (labels == end_id).int()

    in_think = ((start - end).cumsum(dim=-1) > 0).to(torch.float32)  # 0.0 / 1.0
    has_think = (start.sum(dim=-1, keepdim=True) > 0)                # [batch, 1]
    return torch.where(has_think, in_think, torch.full_like(in_think, 2.0))


def compute_think_answer_split(args, losses, loss_mask, think_mask):
    """Compute per-sample and global think/answer loss/token tensors.
    """
    ...
    if not getattr(args, 'log_think_loss', False):
        return None
    interval = getattr(args, 'log_task_loss_interval', -1)
    if interval == -1 or (args.curr_iteration + 1) % interval != 0:
        return None

    think_mask_f = (think_mask == 1.0).float()
    answer_mask_f = (think_mask == 0.0).float()
    masked_losses = losses * loss_mask  # 2D, before flatten

    think_losses = torch.sum(masked_losses * think_mask_f, dim=-1).clone().detach()
    think_tokens = torch.sum(loss_mask * think_mask_f, dim=-1).clone().detach()
    answer_losses = torch.sum(masked_losses * answer_mask_f, dim=-1).clone().detach()
    answer_tokens = torch.sum(loss_mask * answer_mask_f, dim=-1).clone().detach()

    think_report = torch.cat([
        think_losses.sum().view(1),
        think_tokens.sum().to(torch.int).view(1),
    ])
    answer_report = torch.cat([
        answer_losses.sum().view(1),
        answer_tokens.sum().to(torch.int).view(1),
    ])

    return {
        'think_losses_per_sample': think_losses,
        'think_tokens_per_sample': think_tokens,
        'answer_losses_per_sample': answer_losses,
        'answer_tokens_per_sample': answer_tokens,
        'think_report': think_report,
        'answer_report': answer_report,
    }


def add_per_task_think_answer(report, split, task_num, dataset_id):
    """Scatter per-sample think/answer sums into per-task buckets and write
    `task_think_*_list` / `task_answer_*_list` into `report` (consumed by
    `training.py` to produce `lm_loss_think/<ds>` and `lm_loss_answer/<ds>`
    TB tags). No-op when `split` is `None`.
    """
    if split is None:
        return

    def _scatter(values):
        out = torch.zeros((task_num, *dataset_id.shape), device=dataset_id.device)
        out.scatter_(0, dataset_id.unsqueeze(0), values.unsqueeze(0))
        return out.sum(dim=-1)

    report['task_think_losses_list'] = _scatter(split['think_losses_per_sample'])
    report['task_think_tokens_list'] = _scatter(split['think_tokens_per_sample'])
    report['task_answer_losses_list'] = _scatter(split['answer_losses_per_sample'])
    report['task_answer_tokens_list'] = _scatter(split['answer_tokens_per_sample'])


def add_global_think_answer(report, split):
    """Add the two scalar keys `lm loss think` / `lm loss answer` to `report`.
    Names are chosen to share the `lm loss` prefix so they sort adjacent in
    TensorBoard. No-op when `split` is `None`.
    """
    if split is None:
        return
    report['lm loss think'] = split['think_report']
    report['lm loss answer'] = split['answer_report']
